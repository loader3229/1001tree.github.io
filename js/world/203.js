addLayer("203", {
    ...
    tabFormat: {
        点击墙: {
            content: [
                ...
                ["display-text", function () {
                    return `你为了击破当前墙点击了<h2 class="p2pt"> ${formatWhole(player._203.click)} </h2>次`
                }],
                "blank",
                ["clickable",11],
                "blank",
    ...
    subpower() {
        ...
        if (inChallenge(this.layer, 11)) spow = spow.mul(Decimal.pow(0.97, player._203.subclick.max(0)))
    ...
    m2effect() {
        ...
        if (hasChallenge(this.layer, 11)) eff = eff.mul(player._203.subclick.add(1).ln().div(2).add(1))
    ...
    calcmaxhp() {
        ...
        if (player._203.wallbreak.gte(70)) return new Decimal("1.79e308")
        if (player._203.wallbreak.lte(29)) return getYFromOrderedPoints([[0, 10], [10, 100], [20, 1000], [30, 100000]], player._203.wallbreak.add(1)).pow(p)
        return Decimal.pow(getYFromOrderedPoints([[30, 2], [40, 2.5], [50, 3.25], [60, 4.5], [70, 7]], player._203.wallbreak.add(1)), player._203.wallbreak).pow(p)
    },
    divpower() {
        eff = new Decimal(3)
        ...
        eff = eff.times(Decimal.pow(inChallenge(this.layer, 21) ? 0.667 : hasChallenge(this.layer, 21) ? 0.95 : 0.925, player._203.divclick.max(0)))
        return eff.max(1)
    },
    chalcomp() {
        c = _D0
    ...
    clickables: {
        11: {
            title() { return `点击墙 #${formatWhole(player._203.wallbreak)}` },
            display() { return `HP: ${format(player._203.hp)}/${format(player._203.maxhp)}` },
            onClick() {
                player._203.click = player._203.click.add(1)
                if (player._203.mode == 1) {
                    player._203.hp = player._203.hp.sub(layers[this.layer].subpower())
                    if (hasMilestone(this.layer, 5) && (!inChallenge(this.layer, 21))) player._203.divclick = player._203.divclick.sub(hasMilestone(this.layer, 10) ? 0.08 : hasMilestone(this.layer, 7) ? 0.05 : 0.025)
                    if (layers[this.layer].subpower().gt(1)) player._203.subclick = player._203.subclick.add(1)
                } else {
                    player._203.hp = player._203.hp.div(layers[this.layer].divpower())
                    if (layers[this.layer].divpower().gt(1)) player._203.divclick = player._203.divclick.add(1)
                }
                if (player._203.hp.lte(0)) {
                    player._203.maxhp = layers[this.layer].calcmaxhp()
                    player._203.wallbreak = player._203.wallbreak.add(1)
                    player._203.hp = player._203.maxhp
                    player[this.layer].points = player[this.layer].points.add(player._203.wallbreak)
                    player._203.maxpoints = player._203.maxpoints.add(player._203.wallbreak)
                    player._203.divclick = _D0
                    player._203.subclick = _D0
                    player._203.click = _D0
                }
            },
            onHold() {
                this.onClick()
            },
            canClick() { return player._203.wallbreak.lte(70) },
            style: { "background-color"() { return player._203.wallbreak.eq(71) ? "#12a31c" : "hsl(40,50%,50%)" } }
        },
        21: {
            title() { return `减法器` },
            ...
            canClick() { return player._203.mode == 2 },
            style: { "width": "92.5px", "min-height": "75px", "background-color"() { return player._203.mode == 1 ? "#12a31c" : "hsl(40,50%,50%)" } },
        },
        22: {
            title() { return `除法器` },
            ...
            canClick() { return player._203.mode == 1 },
            unlocked() { return hasMilestone(this.layer, 3) },
            style: { "width": "92.5px",  "min-height": "75px", "background-color"() { return player._203.mode == 2 ? "#12a31c" :  "hsl(40,50%,50%)" } },
        },
        31: {
            ...
            onClick() {
                ...
                player[this.layer].points = player._203.maxpoints
    ...
    challenges: {
        11: {
            ...
            canComplete() { return player._203.wallbreak.gte(38) },
            onEnter() {
        ...
        12: {
            ...
            canComplete() { return player._203.wallbreak.gte(49) },
            onEnter() {
                layers[this.layer].clickwallReset()
            },
            onExit() {
        ...
        21: {
            ...
            canComplete() { return player._203.wallbreak.gte(43) },
            onEnter() {
                layers[this.layer].clickwallReset()
            },
            onExit() {
        ...
        22: {
            ...
            canComplete() { return player._203.wallbreak.gte(48) },
            onEnter() {
                layers[this.layer].clickwallReset()
            },
            onExit() {
    ...
    milestones: {
        ...
        8: {
            ...
            done() { return player._203.maxpoints.gte(1326) },
        ...
        9: {
            ...
            done() { return player._203.maxpoints.gte(1953) },
        ...
        10: {
            ...
            done() { return player._203.maxpoints.gte(2346) },
    ...
    clickwallReset() {
        player._203.wallbreak = _D0
        player._203.mode = 1
        player._203.maxhp = _D10
        player._203.hp = _D10
        player._203.maxpoints = _D0
        player[this.layer].points = _D0
        player._203.divclick = _D0
        player._203.subclick = _D0
        player._203.timeplayed = _D0
        setBuyableAmount(this.layer, 11, _D0)
        setBuyableAmount(this.layer, 12, _D0)
        setBuyableAmount(this.layer, 13, _D0)
