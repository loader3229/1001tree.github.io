addLayer("102", {
    ...
    update(diff) {
        ...
        player._102.dB = player._102.dB.mul(_D(0.35).pow(diff))
        if (player._102.dB.lte(0.5)) player._102.dB = _D0
        ...
        if (player._102.tickt.gte(this.getTickTime())) {
            ...
            if (ic == 31) {
                ...
                for (let i = 0; i < meffect.length; i++) {
                    player._102.mul[i] = player._102.mul[i].add(meffect[i])
                }
        ...
        if (ic) {
            ...
            player._102.t = player._102.t.add(diff)
        }
    },
    getTickTime() {
        let s = _D1
    ...
    tabFormat: {
        point: {
            content: [
                ...
                ["display-text", function () {
                    return `挑战已经持续了 ${format(player._102.t)} 秒`
                }],
                ["display-text", function () {
                    if (layers[this.layer].getTickTime().gte(1)) return `距离下一刻 ${formatTime(layers[this.layer].getTickTime().sub(player._102.tickt))}`
                }],
                "blank",
                ["clickable", 11],
                "blank",
        ...
        w3: {
            ...
            unlocked() {
                return inChallenge(102, 21) && hasUpgrade(102, 12)
            }
        },
        w4: {
            content: [
            ...
            unlocked() {
                return inChallenge(102, 22) && hasUpgrade(102, 12)
            }
        },
        w5: {
            content: [
                ...
                ["display-text", function () {
                    return `<div style="
                        ...
                        background: #111111;
                    ">
                    <span class="p1">${format(player._102.mul[0])} </span>×
                    <span class="p2">${format(player._102.mul[1])} </span>×
                    <span class="p3">${format(player._102.mul[2])} </span>×
                    <span class="p4">${format(player._102.mul[3])} </span>×
                    <span class="p5">${format(player._102.mul[4])} </span>×
                    <span class="p6">${format(player._102.mul[5])} </span>×
                    <span class="p7">${format(player._102.mul[6])} </span>×
                    <span class="p8">${format(player._102.mul[7])} </span>×
                    <span class="p9">${format(player._102.mul[8])} </span>
                    <br>
            ...
            unlocked() {
                return inChallenge(102, 31) && hasUpgrade(102, 12)
            }
        },
        c3: {
            content: [
            ...
            unlocked() {
                return inChallenge(102, 121) && hasUpgrade(102, 12)
            }
        },
        c4: {
            content: [
            ...
            unlocked() {
                return inChallenge(102, 122) && hasUpgrade(102, 12)
            }
        }
    },
    getPoint() {
        ...
        else if (ic == 21) p = _D(divNum(getEffect(this.layer, 2111, 10000).mul(_D(2).pow(player._102.t))))
    ...
    grid: {
        ...
        getEffect(data, id) {
            ...
            return d.mul(data).pow(
                decimalMax(
                    getEffect(this.layer, 3141, _D1),
                    getEffect(this.layer, 3142, _D1),
                    getEffect(this.layer, 3143, _D1),
                    getEffect(this.layer, 3144, _D1),
                    getEffect(this.layer, 3145, _D1),
                    getEffect(this.layer, 3146, _D1),
                )
            )
        },
        onClick(data, id) {
    ...
    bars: {
        dB: {
            direction: RIGHT,
            width() {
                return 40 + player._102.dB.div(25).pow(0.25).mul(25).mul(25).toNumber()
            },
            height: 40,
            display() {
                return player._102.dB.gte(0.5) ? `<span style="color:var(--points)">${formatWhole(player._102.dB)}斤鸭梨</span>` : "🤔"
            },
            progress() {
                return player._102.dB.mul(7).pow(1.25).div(7).div(20)
            },
    ...
    upgrades: {
        11: {
            ...
            effect() {
                if (layers[this.layer].getChallenge() == 111) e = player._102.t.add(1).pow(-0.5)
                else e = player._102.t.add(1).pow(0.5)
    ...
    clickables: {
        ...
        12: {
            ...
            onClick() {
                ...
                player._102.dB = player._102.dB.add(1)
            },
            unlocked() { return inChallenge(this.layer, 121) && hasUpgrade(this.layer, 12) }
        },
        13: {
    ...
    resetGame() {
        ...
        player._102.t = _D0
        player._102.tickt = _D0
        player._102.keyseed = Date.now()
        player._102.mul = [_D1, _D1, _D1, _D1, _D1, _D1, _D1, _D1, _D1]
        for (let i = 1; i < 4; i++) {
            for (let j = 1; j < 4; j++) {
